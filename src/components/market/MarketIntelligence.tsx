...
export const MarketIntelligence: React.FC = () => {
  ...
  const [editingAlert, setEditingAlert] = useState<PriceAlert | undefined>(undefined);
  const [showContactSeller, setShowContactSeller] = useState(false);
  const [selectedSeller, setSelectedSeller] = useState({ id: '', name: '' });
  ...
  const [reviewTarget, setReviewTarget] = useState({ name: '', type: 'seller' as 'seller' | 'product' });
  ...
  const handleViewChart = (crop: string) => {
    setSelectedCrop(crop);
    setShowPriceChart(true);
  };

  const handleContactSeller = (listingId: string, sellerName: string) => {
    setSelectedSeller({ id: listingId, name: sellerName });
  ...
  const handleRateSeller = (sellerName: string) => {
    setReviewTarget({ name: sellerName, type: 'seller' });
    setShowReview(true);
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <Button variant="outline" className="flex items-center gap-2" onClick={() => setShowSettings(true)}>
            <Settings className="h-4 w-4" />
            Settings
          </Button>
          <Button className="flex items-center gap-2" onClick={handleRefreshData}>
      ...
      <Tabs defaultValue="prices" className="space-y-6">
                ...
                <CardContent className="pt-4">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex gap-2 ml-4">
                      ...
                      <Button variant="outline" size="sm" onClick={() => handleViewChart(price.crop)}>
        ...
        <TabsContent value="marketplace" className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setShowOrderManagement(true)}>
                  ...
                  <div className="flex gap-6">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center justify-between pt-4 border-t">
                        ...
                        <div className="flex gap-2">
                          <Button variant="outline" onClick={() => handleContactSeller(listing.id, listing.seller.name)}>
                ...
                <CardContent className="pt-6">
                  ...
                  <div className="flex justify-end gap-2">
                    {order.status === 'delivered' && (
                      <Button variant="outline" size="sm" onClick={() => handleRateSeller(order.seller)}>
                ...
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className={`p-3 rounded-full ${alert.enabled ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-400'}`}>
                        <Bell className="h-6 w-6" />
                      </div>
                      <div>
                        <h3 className="text-lg font-bold">{alert.crop}</h3>
                        <p className="text-gray-600">
                          Alert when price is <span className="font-semibold">{alert.condition} {formatCurrency(alert.targetPrice)}</span>
                        </p>
                        <div className="flex gap-2 mt-1">
                          <Badge variant="outline">{alert.market}</Badge>
                          {alert.notifyVia.map(method => (
                            <Badge key={method} variant="secondary" className="capitalize">{method}</Badge>
                          ))}
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <Button variant="ghost" size="icon" className="text-red-500 hover:text-red-600" onClick={() => handleDeleteAlert(alert.id)}>
      ...
      {/* Dialogs */}
      <MarketSettingsDialog open={showSettings} onOpenChange={setShowSettings} />
      <CreateListingDialog open={showCreateListing} onOpenChange={setShowCreateListing} />
      <PriceAlertDialog
      ...
      />
      <ContactSellerDialog
        open={showContactSeller}
        onOpenChange={setShowContactSeller}
        sellerName={selectedSeller.name}
      />
      <OrderTrackingDialog
        open={showOrderTracking}
        onOpenChange={setShowOrderTracking}
        order={selectedOrder}
      />
      <PriceChartDialog
        open={showPriceChart}
        onOpenChange={setShowPriceChart}
